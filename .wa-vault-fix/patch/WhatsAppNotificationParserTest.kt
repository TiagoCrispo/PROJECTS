...
class WhatsAppNotificationParserTest {
    ...
    fun sixEntriesReplacedByDeletionMarkersProduceSixCandidates() {
        ...
        val afterEntries = beforeEntries.mapIndexed { index, old ->
            Msg(old.sender, "Este mensaje fue eliminado", t + 10_000 + index)
        }
        ...
        assertThat(before.messages.map { it.contentType }).containsExactly(
            ContentType.TEXT, ContentType.TEXT, ContentType.IMAGE, ContentType.VIDEO,
            ContentType.VOICE_NOTE, ContentType.DOCUMENT
        ).inOrder()
        ...
        assertThat(batch.candidates.map { it.fingerprint }).containsExactlyElementsIn(
            before.messages.map { it.fingerprint }
        )
    }
    ...
    fun shiftedNotificationWindowRecoversOnlyDeletedMessages() {
        ...
        val after = parser.parse(notification(
            "Ana", false,
            listOf(
                Msg("Ana", "Este mensaje fue eliminado", t + 50_001),
                Msg("Ana", "Este mensaje fue eliminado", t + 50_002),
                d, e
            ),
            t + 50_000, "chat-ana"
        ))!!
        ...
        val batch = engine.compare(before, after)
        assertThat(batch.candidates.map { it.fingerprint }).containsExactly(
            before.messages[1].fingerprint,
            before.messages[2].fingerprint
        )
    }
    ...
    fun groupsPreserveSendersAndNeverCrossConversation() {
        ...
        val groupBefore = parser.parse(notification(
            "UTN", true,
            listOf(Msg("Juan", "J", t + 1), Msg("Maria", "M", t + 2)),
            t, "group-utn"
        ))!!
        val otherBefore = parser.parse(notification(
            "Familia", true,
            listOf(Msg("Juan", "X", t + 1)),
            t, "group-family"
        ))!!
        val groupAfter = parser.parse(notification(
            "UTN", true,
            listOf(Msg("Juan", "Este mensaje fue eliminado", t + 20_000), Msg("Maria", "M", t + 2)),
            t + 20_000, "group-utn"
        ))!!
    ...
    fun normalNotificationContainsNoDeletionEvidence() {
        ...
        val snapshot = parser.parse(notification(
            "Juan", false, listOf(Msg("Juan", "No borrado", t)), t, "chat-clear"
        ))!!
    ...
    private fun notification(
        ...
        val style = NotificationCompat.MessagingStyle(me)
            .setConversationTitle(if (group) title else null)
        ...
        val n = NotificationCompat.Builder(context, "wa-vault-test")
