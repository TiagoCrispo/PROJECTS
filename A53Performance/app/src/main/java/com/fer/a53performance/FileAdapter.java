...
public final class FileAdapter extends RecyclerView.Adapter<FileAdapter.Holder> {
    ...
    @NonNull @Override public Holder onCreateViewHolder(@NonNull ViewGroup parent,int viewType){
        int p=dp(parent,12);LinearLayout row=new LinearLayout(parent.getContext());row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(p,dp(parent,8),p,dp(parent,8));row.setBackgroundColor(Color.rgb(12,16,20));
