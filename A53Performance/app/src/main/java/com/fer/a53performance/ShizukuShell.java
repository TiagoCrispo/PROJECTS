...
public final class ShizukuShell {
    ...
    public boolean warmUp(long timeoutMs){if(!permissionGranted()||System.currentTimeMillis()<breakerUntil)return false;IPrivilegedService local=service;if(local!=null&&local.asBinder().pingBinder())return true;bindIfNeeded();try{return bindLatch.await(Math.max(300L,timeoutMs),TimeUnit.MILLISECONDS)&&service!=null&&service.asBinder().pingBinder();}catch(InterruptedException e){Thread.currentThread().interrupt();return false;}}
    private void bindIfNeeded(){IPrivilegedService local=service;if(local!=null&&local.asBinder().pingBinder())return;if(System.currentTimeMillis()<breakerUntil||!binding.compareAndSet(false,true))return;synchronized(bindLock){bindLatch=new CountDownLatch(1);try{Shizuku.bindUserService(args(),connection);}catch(Throwable t){binding.set(false);breakerUntil=System.currentTimeMillis()+BREAKER_MS;bindLatch.countDown();}}}
    ...
    public Result listRunningUserPackages(){return callString(()->service.listRunningUserPackages(),2600L,true);}
    public Result listSensitiveUserPackages(){return callString(()->service.listSensitiveUserPackages(),3600L,true);}
    ...
    private Result callString(Callable<String> call,long timeoutMs,boolean retry){
        if(!permissionGranted())return new Result(false,"Shizuku sin permiso",-1);if(!warmUp(1800L))return new Result(false,"UserService no disponible",-5);
        ...
        try{Result r=f.get(timeoutMs,TimeUnit.MILLISECONDS);if(transportFailure(r)&&retry&&reconnectOnce(1600L))return callString(call,timeoutMs,false);if(transportFailure(r))breakerUntil=System.currentTimeMillis()+BREAKER_MS;return r;}catch(Throwable e){f.cancel(true);if(retry&&reconnectOnce(1600L))return callString(call,timeoutMs,false);breakerUntil=System.currentTimeMillis()+BREAKER_MS;return new Result(false,"timeout",-2);}
    }
    private Result callInt(Callable<Integer> call,long timeoutMs,boolean retry){
        if(!permissionGranted())return new Result(false,"Shizuku sin permiso",-1);if(!warmUp(Math.min(1800L,timeoutMs+400L)))return new Result(false,"UserService no disponible",-5);
        ...
        try{Result r=f.get(Math.max(900L,timeoutMs+500L),TimeUnit.MILLISECONDS);if(transportFailure(r)&&retry&&reconnectOnce(1600L))return callInt(call,timeoutMs,false);if(transportFailure(r))breakerUntil=System.currentTimeMillis()+BREAKER_MS;return r;}catch(Throwable e){f.cancel(true);if(retry&&reconnectOnce(1600L))return callInt(call,timeoutMs,false);breakerUntil=System.currentTimeMillis()+BREAKER_MS;return new Result(false,"timeout",-2);}
    }
