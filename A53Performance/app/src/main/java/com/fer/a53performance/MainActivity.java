...
public final class MainActivity extends Activity {
    ...
    private static final int PAGE_SIZE=60, SHIZUKU_REQUEST=5401, PERMISSION_REQUEST=5402;
    ...
    private Button loadMore,deleteSelected,scanButton;
    ...
    @Override protected void onCreate(Bundle state){
        ...
        shell=new ShizukuShell();storage=new StorageRepository(this);thumbnails=new ThumbnailLoader(this);optimizer=new SystemOptimizer(this,shell);
    ...
    private void navigate(String page){
        if(fileList!=null && !"cleaner".equals(page)){saveListAnchor();thumbnails.cancelVisualWork();}
    ...
    private void clearContent(){content.removeAllViews();fileList=null;search=null;filterSpinner=null;sortSpinner=null;loadMore=null;deleteSelected=null;scanButton=null;scanProgress=null;selectionSummary=null;storageSummary=null;scanStatus=null;}
    ...
    private void showHome(){
        ...
        LinearLayout shCard=card();TextView sh=text("Shizuku: "+(shell.permissionGranted()?"Activo":"Sin autorización"),16,shell.permissionGranted()?ACCENT:Color.WHITE,true);shCard.addView(sh);TextView shSub=text(shell.permissionGranted()?"Los ajustes avanzados pueden aplicarse sin bloquear la interfaz.":"Autorízalo para perfiles y cierre real de procesos.",13,MUTED,false);shCard.addView(shSub);if(!shell.permissionGranted()){Button grant=button("Autorizar Shizuku");grant.setOnClickListener(v->shell.requestPermission(SHIZUKU_REQUEST));shCard.addView(grant);}box.addView(shCard);
    ...
    private void showCleaner(){
        ...
        scanButton=button("Escanear almacenamiento");scanButton.setOnClickListener(v->startScan(false));top.addView(scanButton);box.addView(top);
        ...
        LinearLayout filters=new LinearLayout(this);filters.setOrientation(LinearLayout.HORIZONTAL);filterSpinner=spinner(new String[]{"Todo","Imágenes","Videos","Audio","Grandes"});sortSpinner=spinner(new String[]{"Más recientes","Más grandes","Más pequeños","Nombre"});filters.addView(filterSpinner,new LinearLayout.LayoutParams(0,dp(48),1f));filters.addView(sortSpinner,new LinearLayout.LayoutParams(0,dp(48),1f));box.addView(filters);
        filterSpinner.setSelection(prefs.getInt("clean_filter",0));sortSpinner.setSelection(prefs.getInt("clean_sort",0));
        thumbnails.cancelVisualWork();fileAdapter=new FileAdapter(thumbnails,(count,bytes)->{if(selectionSummary!=null){selectionSummary.setText(count==0?"Sin selección":count+" seleccionados · "+FileAdapter.formatBytes(bytes));if(deleteSelected!=null)deleteSelected.setEnabled(count>0);}});
        ...
        LinearLayout actions=new LinearLayout(this);actions.setOrientation(LinearLayout.HORIZONTAL);loadMore=button("Mostrar más");deleteSelected=button("Eliminar");deleteSelected.setEnabled(false);actions.addView(loadMore,new LinearLayout.LayoutParams(0,dp(48),1f));actions.addView(deleteSelected,new LinearLayout.LayoutParams(0,dp(48),1f));box.addView(actions);
        loadMore.setOnClickListener(v->appendPage());deleteSelected.setOnClickListener(v->deleteSelection());
        search.addTextChangedListener(new android.text.TextWatcher(){public void beforeTextChanged(CharSequence s,int st,int c,int a){}public void onTextChanged(CharSequence s,int st,int b,int c){if(searchRunnable!=null)main.removeCallbacks(searchRunnable);searchRunnable=()->{prefs.edit().putString("clean_query",search.getText().toString()).apply();visibleLimit=PAGE_SIZE;applyFilter(false);};main.postDelayed(searchRunnable,260);}public void afterTextChanged(android.text.Editable e){}});
        filterSpinner.setOnItemSelectedListener(simpleSelection(()->{prefs.edit().putInt("clean_filter",filterSpinner.getSelectedItemPosition()).apply();visibleLimit=PAGE_SIZE;applyFilter(false);}));
        sortSpinner.setOnItemSelectedListener(simpleSelection(()->{prefs.edit().putInt("clean_sort",sortSpinner.getSelectedItemPosition()).apply();applyFilter(true);}));
        if(storage.snapshot().isEmpty())startScan(true);else{visibleLimit=Math.max(PAGE_SIZE,savedVisible);applyFilter(false);restoreListAnchorLater();}
    }
    ...
    private void startScan(boolean quiet){
        if(scanRunning){storage.cancelScan();scanRunning=false;}
        ...
        storage.scanAsync((gen,items,error)->runOnUiThread(()->{
            scanRunning=false;if(!currentPage.equals("cleaner"))return;
            scanButton.setEnabled(true);scanButton.setText("Escanear almacenamiento");scanProgress.setVisibility(View.GONE);visibleLimit=Math.max(PAGE_SIZE,savedVisible);applyFilter(false);scanStatus.setText(items.size()+" archivos indexados"+(error==null?"":" · aviso: "+error));restoreListAnchorLater();
        }));
    }
    ...
    private void applyFilter(boolean preserveAnchor){
        ...
        filtered.clear();for(StorageItem x:storage.snapshot()){
            boolean type=switch(f){case 1->x.isImage();case 2->x.isVideo();case 3->x.isAudio();case 4->x.isLarge();default->true;};
        ...
        if(preserveAnchor&&anchor>=0){int a=Math.min(anchor,Math.max(0,fileAdapter.getItemCount()-1));int off=offset;fileList.post(()->((LinearLayoutManager)fileList.getLayoutManager()).scrollToPositionWithOffset(a,off));}
    }
    ...
    private void updateCleanerSummary(){if(storageSummary!=null)storageSummary.setText("Libre: "+FileAdapter.formatBytes(StorageRepository.freeBytes())+"  ·  "+fileAdapter.displayedCount()+"/"+filtered.size()+" visibles");if(loadMore!=null)loadMore.setEnabled(fileAdapter.displayedCount()<filtered.size());savedVisible=Math.max(PAGE_SIZE,fileAdapter.displayedCount());}

    private void deleteSelection(){
        List<StorageItem> selected=fileAdapter.selectedItems();if(selected.isEmpty())return;int beforeCount=fileAdapter.displayedCount();deleteSelected.setEnabled(false);deleteSelected.setText("Eliminando…");
        storage.deleteAsync(selected,(deleted,failed)->runOnUiThread(()->{
            if(!currentPage.equals("cleaner"))return;fileAdapter.removeAll(deleted);visibleLimit=Math.max(PAGE_SIZE,beforeCount);applyFilter(true);deleteSelected.setText("Eliminar");deleteSelected.setEnabled(false);updateCleanerSummary();scanStatus.setText(deleted.size()+" eliminados"+(failed.isEmpty()?"":" · "+failed.size()+" no pudieron borrarse"));
        }));
    }
    ...
    private void showSettings(){
        ...
        LinearLayout protectedCard=card();protectedCard.addView(text("Apps protegidas por defecto",16,Color.WHITE,true));protectedCard.addView(text("Gmail · Mensajes Google/Samsung · Reloj Google/Samsung · Brave · ChatGPT · Samsung Voice Recorder · Teléfono/Contactos · servicios críticos del sistema",13,MUTED,false));protectedCard.addView(text("Esta protección se centraliza y se aplica a acciones por app. Los cambios globales de Android, como Battery Saver o Data Saver, siguen siendo globales.",12,MUTED,false));box.addView(protectedCard);
        LinearLayout about=card();about.addView(text("v1.15.4 PERFORMANCE / STABILITY",16,ACCENT,true));about.addView(text("RecyclerView virtualizado, paginación real, cola limitada de miniaturas, cancelación por memoria/pantalla, búsqueda con debounce y tareas pesadas serializadas.",13,MUTED,false));box.addView(about);
    }
    ...
    private void restoreListAnchorLater(){if(fileList==null||savedAnchor<0)return;int a=Math.min(savedAnchor,Math.max(0,fileAdapter.getItemCount()-1)),o=savedAnchorOffset;fileList.post(()->((LinearLayoutManager)fileList.getLayoutManager()).scrollToPositionWithOffset(a,o));savedAnchor=-1;}
    ...
    @Override public void onTrimMemory(int level){super.onTrimMemory(level);thumbnails.trim(level);if(level>=TRIM_MEMORY_RUNNING_LOW&&fileAdapter!=null&&fileAdapter.displayedCount()>PAGE_SIZE*2){saveListAnchor();visibleLimit=Math.max(PAGE_SIZE,Math.min(fileAdapter.displayedCount(),PAGE_SIZE*2));applyFilter(true);}}
    @Override protected void onDestroy(){try{Shizuku.removeRequestPermissionResultListener(shizukuPermissionListener);}catch(Throwable ignored){}storage.shutdown();thumbnails.shutdown();optimizer.shutdown();shell.shutdown();super.onDestroy();}
