...
public final class StabilityInstrumentedTest {
    @Test public void tenThousandRowsKeepUniqueStableIdsAndSelectionAcrossPages(){
        ...
        try{
            ...
            List<StorageItem> filtered=CleanerController.filterAndSort(items,"file_9",0,0,Collections.emptySet(),Collections.emptySet());assertTrue(filtered.size()>100);
            adapter.setSelection(items.subList(0,120));adapter.replace(new ArrayList<>(items.subList(0,60)));assertEquals(60,adapter.getItemCount());assertEquals(120,adapter.selectedKeys().size());adapter.append(new ArrayList<>(items.subList(60,120)));assertEquals(120,adapter.getItemCount());assertEquals(120,adapter.selectedItems(items).size());
    ...
    @Test public void duplicateAnalyzerBuildsSafeKeepOneGroup()throws Exception{
        Context context=InstrumentationRegistry.getInstrumentation().getTargetContext();File a=new File(context.getCacheDir(),"dup_a.bin"),b=new File(context.getCacheDir(),"dup_b.bin"),c=new File(context.getCacheDir(),"dup_c.bin");byte[] same=new byte[80*1024],different=new byte[80*1024];for(int i=0;i<same.length;i++){same[i]=(byte)(i*31);different[i]=(byte)(i*17+7);}try(FileOutputStream out=new FileOutputStream(a)){out.write(same);}try(FileOutputStream out=new FileOutputStream(b)){out.write(same);}try(FileOutputStream out=new FileOutputStream(c)){out.write(different);}
        StorageAnalyzer analyzer=new StorageAnalyzer(context);try{
            ArrayList<StorageItem> items=new ArrayList<>();items.add(new StorageItem(0,null,"a",a.getAbsolutePath(),"application/octet-stream",a.length(),a.lastModified()+2));items.add(new StorageItem(0,null,"b",b.getAbsolutePath(),"application/octet-stream",b.length(),b.lastModified()+1));items.add(new StorageItem(0,null,"c",c.getAbsolutePath(),"application/octet-stream",c.length(),c.lastModified()));
            CountDownLatch latch=new CountDownLatch(1);AtomicReference<StorageAnalyzer.DuplicateResult> ref=new AtomicReference<>();analyzer.analyzeDuplicatesAsync(items,new StorageAnalyzer.Callback<>(){public void onPhase(String p){}public void onDone(StorageAnalyzer.DuplicateResult r){ref.set(r);latch.countDown();}});assertTrue(latch.await(15,TimeUnit.SECONDS));StorageAnalyzer.DuplicateResult r=ref.get();assertNotNull(r);assertEquals(1,r.groups);assertEquals(2,r.keys.size());assertEquals(1,r.safeDeleteKeys.size());assertEquals(a.length(),r.recoverableBytes);assertEquals(2,r.groupList.get(0).items.size());assertFalse(r.safeDeleteKeys.contains(r.groupList.get(0).keeperKey));
        }finally{analyzer.shutdown();a.delete();b.delete();c.delete();}
    }

    @Test public void persistentIndexAndCustomProtectionRoundTrip(){
        Context context=InstrumentationRegistry.getInstrumentation().getTargetContext();StorageIndexDb db=new StorageIndexDb(context);String custom="com.example.keepme";
        try{
            StorageItem item=new StorageItem(77L,Uri.parse("content://media/external/file/77"),"persist.bin","/sdcard/Download/persist.bin","application/octet-stream",777L,1700000000777L);db.replaceAll(List.of(item),"sig-test");assertEquals("sig-test",db.signature());assertEquals(1,db.load().size());assertTrue(db.keys().contains(item.stableKey()));db.remove(List.of(item));assertTrue(db.load().isEmpty());
            AppProtection.setUserProtected(context,Set.of(custom));assertTrue(AppProtection.userProtected(context).contains(custom));AppProtection.setUserProtected(context,Set.of());assertFalse(AppProtection.userProtected(context).contains(custom));
        }finally{db.close();}
    }
}
