...
# The old create button contains an inner `if (processing)`; the public-flow cut
# intentionally stops at that marker, so remove the remaining tail deterministically.
orphan='''                if (processing) {\n                            CircularProgressIndicator(Modifier.width(20.dp), strokeWidth = 2.dp)\n                            Spacer(Modifier.width(10.dp))\n                            Text("Cancelar")\n                        } else {\n                            Text("Crear foto")\n                        }\n                    }\n                }\n\n'''
if orphan not in s:
    raise SystemExit('old create-button tail not found')
