...
echo "[3/20] Block 2 NotificationListener regression"; python3 tools/v0530_block2_listener_regression_test.py
echo "[4/20] Block 3 persistence/idempotency regression"; python3 tools/v0530_block3_persistence_regression_test.py
echo "[5/20] Block 3 DB v13->v14 migration"; python3 tools/v0530_block3_db_migration_test.py
echo "[6/20] Block 4 media/storage regression"; python3 tools/v0530_block4_media_regression_test.py
echo "[7/20] Block 4 DB v14->v15 migration"; python3 tools/v0530_block4_db_migration_test.py
echo "[8/20] Block 4 media/storage stress"; python3 tools/v0530_block4_media_stress_test.py
echo "[9/20] Block 5 concurrency/background regression"; python3 tools/v0530_block5_concurrency_background_regression_test.py
echo "[10/20] Block 6 security/Android 16 regression"; python3 tools/v0530_block6_security_android16_regression_test.py

echo "[11/20] lifecycle regression suite"; python3 -m unittest -v tools/test_deletion_state_machine.py

echo "[12/20] DeletionGuard JVM self-test"
...
echo "[13/20] Java delimiter/lexical smoke"
python3 - <<'PYLEX'
...
echo "[15/20] frozen v0.5.26/v0.5.27 invariants"
for v in 26 27; do
...
echo "[16/20] security/privacy source audit"; python3 tools/security_source_audit.py

echo "[17/20] host JUnit signature/type smoke"
...
echo "[18/20] bounded executor runtime self-test"; python3 tools/v0530_block5_executor_backpressure_selftest.py

echo "[19/20] nested catch semantic smoke"; python3 tools/check_nested_catch_redeclaration.py

echo "[20/20] CI YAML / shell syntax"
python3 - <<'PYYAML'
from pathlib import Path
import yaml
